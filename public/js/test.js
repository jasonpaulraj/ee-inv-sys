document.addEventListener('DOMContentLoaded', () => {
    ...
    // We can still optionally reload from api after resetting db, or just use page reload
    ...
    window.resetDb = async function() {
        if(!confirm("This will wipe all data and reseed the entire database. Continue?")) return;
        ...
        try {
            ...
            log(data.message);
            // Optionally just window.location.reload(); but fetching new bounds via ajax is nice
            await loadVariants();
            log('Ready.');
        } catch(e) {
    ...
    window.runTest = async function() {
        ...
        const numRequests = parseInt(document.getElementById('num-requests').value, 10);
        const stock = parseInt(select.options[select.selectedIndex].dataset.stock, 10);
        targetStock = stock;
        
        if (!variantId || isNaN(numRequests) || numRequests <= 0) {
            alert("Invalid input parameters."); return;
        }
        ...
        log(`Initializing ${numRequests} concurrent requests for Variant #${variantId} (Initial Available: ${stock})`);
        ...
        for (let i = 0; i < numRequests; i++) {
            // Generate a random idempotency key for every distinct "simulated user"
        ...
        if (success > targetStock) {
            log(`CRITICAL FAILURE: Oversold! Permitted ${success} reserves on an initial available stock of ${targetStock}.`, true);
        } else if (success === targetStock && targetStock > 0) {
            log(`SUCCESS: Reserved exactly ${targetStock} items. Locking prevented over-selling perfectly.`);
        } else {
            log(`INFO: Not all available stock was reserved (${success}/${targetStock}). This is nominal if requests were very few, or if stock was 0.`);
        }
