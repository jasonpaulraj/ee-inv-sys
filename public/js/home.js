document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize initial prices and stocks
    document.querySelectorAll('.variant-select').forEach(select => {
        ...
        const updatePriceAndStock = () => {
            ...
            if (stockEl) {
                if (stock > 10) {
                    ...
                    stockEl.className = 'text-xs font-semibold mt-0.5 text-emerald-600';
                    btn.disabled = false;
                } else if (stock > 0) {
                    stockEl.textContent = `Only ${stock} left`;
                    stockEl.className = 'text-xs font-semibold mt-0.5 text-amber-500';
                    btn.disabled = false;
                } else {
                    stockEl.textContent = 'Out of Stock';
                    stockEl.className = 'text-xs font-semibold mt-0.5 text-red-500';
                    btn.disabled = true;
                }
            }
        };
        ...
        updatePriceAndStock(); // init
    });

    // Handle Reservation Button logic
    document.querySelectorAll('.btn-reserve').forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    showToast('Item reserved successfully! Redirecting...', 'success');
                    setTimeout(() => {
                        window.location.href = `/checkout/${data.data.reservation_id}`;
                    }, 1000);
                } else {
                    showToast(data.message || 'Failed to reserve item.', 'error');
